...
export default function QuestionForm({ onSubmitEntry, isSubmitting: externalIsSubmitting, onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (onSubmitEntry) {
        await onSubmitEntry({ name, type, content });
      }
    ...
    <div className="flex flex-col h-full max-h-full space-y-2.5 min-h-0 pb-1 pr-1 w-full justify-end">
      ...
      >
        <div className="flex flex-col flex-1 min-h-0 overflow-y-auto custom-scrollbar">
          ...
          <div className="border-b-2 border-black/10 pb-3 mb-3 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="p-2 bg-[#E38B80] border border-black rounded-lg shadow-[1px_1px_0px_0px_rgba(0,0,0,1)]">
          ...
          <div className="space-y-3.5 flex flex-col flex-1">
            ...
            <div className="space-y-1.5 shrink-0">
              <label className="text-sm font-black text-gray-900 flex items-center gap-2">
                <User className="w-4 h-4 text-[#B35A53]" /> Name <span className="text-gray-500 font-bold text-xs">(Optional)</span>
              </label>
              <input
                type="text"
                ...
                className="w-full px-3 py-2 bg-[#FFFDF9] border-2 border-black/80 rounded-xl text-sm sm:text-base font-bold placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#B35A53] transition-all"
              />
            </div>
            ...
            <div className="space-y-1.5 shrink-0">
              <label className="text-sm font-black text-gray-900 flex items-center gap-2">
                <Tag className="w-4 h-4 text-[#B35A53]" /> Entry Category
              </label>
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center justify-center gap-1.5 py-2 px-1.5 rounded-xl border-2 font-black text-xs sm:text-sm transition-all cursor-pointer ${
                        isSelected
            ...
            {/* Message Input - Takes up remaining vertical space dynamically */}
            <div className="space-y-1.5 flex flex-col flex-1 min-h-[120px]">
              <label className="text-sm font-black text-gray-900 flex items-center gap-2 shrink-0">
                <HelpCircle className="w-4 h-4 text-[#B35A53]" /> Your Message
              </label>
              <textarea
                ...
                required
                className="w-full p-3 bg-[#FFFDF9] border-2 border-black/80 rounded-xl text-sm sm:text-base font-bold placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-[#B35A53] resize-none transition-all flex-1 h-full"
              />
            </div>
          </div>
        </div>
        ...
        <motion.button
          ...
          className="w-full inline-flex items-center justify-center gap-2 bg-[#B35A53] hover:bg-[#a04e47] text-white font-black py-3 px-4 rounded-xl border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transition-all cursor-pointer disabled:opacity-50 mt-3 text-sm sm:text-base shrink-0"
        >
